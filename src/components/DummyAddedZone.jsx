...
const DummyAddedZone = ({ zone, onUpdate }) => {
  const is12Hour = true;
  const dateFormat = false;
  const [time, setTime] = useState(updateTime(zone, is12Hour));
  const [error, setError] = useState(null);
  const homeZone = localStorage.getItem("homeLocation");
  ...
  const localHour = () => {
    let hour = updateTime(homeZone, true).hour;
  ...
  const handleDeleteZone = async () => {
    ...
    const newZone = pAddedZone.filter((z) => z !== zone);
    localStorage.setItem("addedZones", JSON.stringify(newZone));
    onUpdate();
  };
